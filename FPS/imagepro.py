...
def blackPointSelect():
	...
	#cv.ADAPTIVE_THRESH_MEAN_C, cv.THRESH_BINARY, 3, 5
	#_, img = cv.threshold(img, 0, 255, cv.ADAPTIVE_THRESH_MEAN_C, cv.THRESH_BINARY)
	_, img = cv.threshold(img,-255, 255,cv.THRESH_TOZERO)
